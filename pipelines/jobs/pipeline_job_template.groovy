...
pipelineJob("${BUILD_FOLDER}/${JOB_NAME}") {
    ...
    parameters {
        ...
        choiceParam('releaseType', ['Nightly Without Publish', 'Nightly', 'Weekly', 'Release'], 'Nightly - release a standard nightly build.<br/>Nightly Without Publish - run a nightly but do not publish.<br/>Weekly - release a standard weekly build, run with extended tests.<br/>Release - this is a release, this will need to be manually promoted.')
        stringParam('overridePublishName', "", '<strong>REQUIRED for OpenJ9</strong>: Name that determines the publish name (and is used by the meta-data file), defaults to scmReference(remove ending _adopt or _adoptium if present).<br/>Nightly builds: Leave blank (defaults to a date_time stamp).<br/>OpenJ9 Release build Java 8 example <code>jdk8u192-b12_openj9-0.12.1</code> and for OpenJ9 Java 11 example <code>jdk-11.0.2+9_openj9-0.12.1</code>.')
        stringParam('scmReference', "", 'Tag name or Branch name from which to build. Nightly builds: Defaults to, Hotspot=dev, OpenJ9=openj9, others=master.</br>Release builds: For hotspot JDK8 this would be the OpenJDK tag, for hotspot JDK11+ this would be the Adoptium merge tag for the desired OpenJDK tag eg.jdk-11.0.4+10_adopt or jdk-11.0.4+10_adoptium, and for OpenJ9 this will be the release branch, eg.openj9-0.14.0.')
        ...
        booleanParam('cleanWorkspaceBuildOutputAfterBuild', false, "Clean out the workspace/build/src/build and workspace/target output only, after the build")
